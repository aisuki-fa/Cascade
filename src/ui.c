...
void ui_draw_sidebar(SimState* sim, UIState* ui, ObstacleList* obs) {
    ...
    if (GuiButton(grd, "Fluidity")) ui->show_ms = (ui->show_ms + 1) % 4;                // toggle overlay//TT%3 to for converting 3 to 0 
    ...
    if (ui->show_ms==3) DrawRectangleLinesEx(grd, 15.0f, t.accent);
