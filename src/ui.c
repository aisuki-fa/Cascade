...
void ui_draw_sidebar(SimState* sim, UIState* ui, ObstacleList* obs) {
    ...
    DrawRectangle(0, 0, SIDEBAR_W, WINDOW_H, t.panel);                         // sidebar fill or background
    ...
    // ── Slider style ────────────────────────────────────────────
    GuiSetStyle(SLIDER, BASE_COLOR_NORMAL,   ColorToGui((Color){t.section.r, t.section.g, t.section.b, 255})); // forcing alpha 255
    ...
    GuiSetFont(font_ui);                                                       // set font back to default size for this section
